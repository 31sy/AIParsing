...
def evaluation(dataset, all_boxes, all_segms, all_keyps, all_parss, all_pscores, all_uvs, clean_up=True):
    ...
    for iou_type in iou_types:
        if iou_type == "parsing":
            eval_ap = cfg.AIParsing.EVAL_AP
            num_parsing = cfg.AIParsing.NUM_PARSING
            assert len(cfg.DATASETS.TEST) == 1, 'Parsing only support one dataset now'
            im_dir = dataset_catalog.get_im_dir(cfg.DATASETS.TEST[0])
            ann_fn = dataset_catalog.get_ann_fn(cfg.DATASETS.TEST[0])
            res = evaluate_parsing(
                coco_results[iou_type], eval_ap, cfg.AIParsing.SCORE_THRESH, num_parsing, im_dir, ann_fn, output_folder
            )
