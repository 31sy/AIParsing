...
from maskrcnn_benchmark.structures.boxlist_ops import cat_boxlist, boxlist_nms, \
...
def im_detect_bbox(model, ims,image_ids):
    box_results = [[] for _ in range(len(image_ids))]
    features = []
    results, net_imgs_size, blob_conv = im_detect_bbox_net(model, ims)
    # if cfg.RPN.RPN_ONLY:
    #     return results, None
...
def im_detect_bbox_net(model, ims,flip=False, size=None):
    net_imgs_size = []
    results = []
    #ims_blob = get_blob(ims, target_scale, target_max_size, flip)
    blob_conv, _results = model.box_net(ims)
